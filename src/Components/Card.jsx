...
const ActionButtons = ({ onAddToCart, isInCart, isWishlist, onToggleWishlist, isCompare, onToggleCompare, isMobile = false }) => (
  <div className='flex justify-between items-center gap-1.5'>
    <p 
      ...
      className='bg-[#f3f3f3] rounded-[8px] w-[38px] h-[38px] flex justify-center items-center cursor-pointer select-none shrink-0'
    >
      {isWishlist ? (
    ...
    {isInCart ? (
      <Link 
        ...
        className='border border-[#22c55e] bg-white flex items-center gap-1.5 h-[38px] text-[#22c55e] font-[Montserrat,_sans-serif] justify-center flex-1 rounded-[8px] cursor-pointer transition-colors duration-200 no-underline'
      >
        <img src={greenBasket} alt="greenbasket" className='w-4 h-4' />
        <span className='text-[13px] font-[500]'>Səbətdə</span>
      </Link>
    ) : (
      <p 
        onClick={onAddToCart}
        className='bg-[#ff003c] flex items-center gap-1 h-[38px] text-white font-[Montserrat,_sans-serif] justify-center flex-1 rounded-[8px] cursor-pointer transition-colors duration-200'
      >
        <img src={whitebasket} alt="whitebasket" />
        <span className='text-[13px] font-[500]'>Səbətə at</span>
      </p>
    )}
  </div>
);

function Card({ products: propProducts }) {
      ...
      {products && products.map((product, i) => {
        ...
        return (
          <div 
            ...
            className={`product-card snap-start group/card bg-white p-4 w-70 shadow-md cursor-pointer transition-all duration-300 relative shrink-0 ${
          ...
          >
            ...
            {defaultVariant && defaultVariant.images && (
              <div className='flex relative items-center justify-center'>
                <img 
                  ...
                  alt={product.title} 
                  className="w-[210px] h-64 object-contain rounded"
                  onClick={() => handleCardClick(product, defaultVariant)}
                />

            
                {numericDiscount > 0 && (
                  <div className='bg-[#ff003c] text-white w-[50px] absolute top-[30px] right-[6px] h-[50px] font-[700] font-[Montserrat,_sans-serif] flex justify-center items-center rounded-[100%]'>
                ...
                {product.delivery && (
                  <span className='text-[#17a539] bg-[#edf8f0] absolute left-0 top-0 text-[.7em] font-[500] px-2 font-[Montserrat,_sans-serif] whitespace-nowrap flex justify-center items-center gap-1 rounded-[5px]'>
                ...
                {product.video && (
                  <div 
                    ...
                    className='flex items-center bg-white gap-1 border border-[#ff003c] p-1 rounded-[4px] absolute left-0 bottom-[47px] transition-colors duration-300'
                  >
                    <p className='text-[#ff003c] bg-white w-[17px] h-[17px] rounded-[50%] flex items-center justify-center border border-[#ff003c] text-[10px]'>
                      <TbPlayerPlayFilled />
                    </p>
                    <span className='text-[#ff003c] font-[Montserrat,_sans-serif] font-[500] text-[10px]'>
                      Video
                    </span>
                  </div>
                )}
            ...
            <div className="flex gap-2 justify-center items-center mt-2 h-5">
            ...
            {(() => {
              ...
              return (
                <h3 className="text-[#323232] font-[Montserrat,_sans-serif] mt-3 text-[13px] font-[500] line-clamp-2 h-[55px] flex items-center">
                  {product.title}
            ...
            <div className="mt-1 h-[22px]">
              {numericDiscount > 0 && (
                <span className="bg-[#ff003c] text-white text-[12px] font-[500] py-1 px-1.5 rounded-[4px] font-[Montserrat,_sans-serif] inline-block">
                  -{discountAmount}₼
                </span>
              )}
            </div>
            ...
            <div className="mt-1 h-[42px] flex flex-col justify-end">
              <div className="h-[18px]">
                {numericDiscount > 0 && (
                  <span className="line-through text-[#777777] font-[Montserrat,_sans-serif] font-[550] text-[.9em]">{product.price} ₼</span>
                )}
              </div>
              <div className='flex justify-between items-center'>
                <span className="text-[#ff003c] font-[650] font-[Montserrat,_sans-serif] mr-2">
                  {finalPrice} ₼
                </span>
                {product.month && (
                  <span className='font-[Montserrat,_sans-serif] font-[500] text-[13px]'>0% {product.month} ay</span>
                )}
              </div>
            </div>
